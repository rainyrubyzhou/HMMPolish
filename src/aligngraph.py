...
:param b: second node
:param c: third node
:return: a corresponding peptide of sequence abc
"""

    seq = a + b + c

    return codon_table[seq]


def aminoacidindex(aminoacid):
    """
    :param aminoacid: a letter represent the amino acid
    :return: index: that can track the background score of amino acid
    """
    dict_aa = {"A": 0, "C": 1, "D": 2, "E": 3, "F": 4, "G": 5, "H": 6, "I": 7, "K": 8, "L": 9,
               "M": 10, "N": 11, "P": 12, "Q": 13, "R": 14, "S": 15, "T": 16, "V": 17, "W": 18, "Y": 19, "*": 20}
    index = dict_aa[aminoacid.upper()]
    return index
...
def rev_com(sequence):
    ...
    :return: revcom_seq: reverse complement sequence of input
    """
    revcom_seq = ""
    for i in sequence[::-1]:
        revcom_seq += rev_table[i]
